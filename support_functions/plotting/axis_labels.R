axis_labels <- function(n_row, n_col, numbering_interval = 5, axis_label_resize = 1) {
  ...
  if (axis_label_resize != 0) {
    # left labels
    text(rep(1, floor(n_row/numbering_interval)), c(n_row, seq(n_row - (numbering_interval-1), 1, -numbering_interval)), labels = make_sequence(1, n_row), cex = 1.5 * axis_label_resize, pos = 2)
    # right labels
    text(rep(n_col, floor(n_row/numbering_interval)), c(n_row, seq(n_row - (numbering_interval-1), 1, -numbering_interval)), labels = make_sequence(1, n_row), cex = 1.5 * axis_label_resize, pos = 4)
    # top labels
